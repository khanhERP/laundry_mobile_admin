...
export function OrderManagement() {
  ...
  const { data: orders, isLoading: ordersLoading } = useQuery({
    queryKey: ["https://api-laundry-mobile.edpos.vn/api/orders", "table"],
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        "https://api-laundry-mobile.edpos.vn/api/orders?salesChannel=table",
      );
      if (!response.ok) {
  ...
  const { data: storeSettings } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest("GET", "https://api-laundry-mobile.edpos.vn/api/store-settings");
  ...
  const { data: orderItems, isLoading: orderItemsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiRequest(
          "GET",
          `https://api-laundry-mobile.edpos.vn/api/order-items/${selectedOrder.id}`,
        );
        if (!response.ok) {
          console.error(
  ...
  const updateOrderStatusMutation = useMutation({
    mutationFn: ({ orderId, status }: { orderId: number; status: string }) =>
      apiRequest("PUT", `https://api-laundry-mobile.edpos.vn/api/orders/${orderId}/status`, { status }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["https://api-laundry-mobile.edpos.vn/api/orders"] });
      queryClient.invalidateQueries({ queryKey: ["https://api-laundry-mobile.edpos.vn/api/tables"] });
      toast({
  ...
  const completeOrderPayment = async (orderId: number, paymentData: any) => {
    ...
    try {
      ...
      console.log("🔍 API Call Details:", {
        url: `https://api-laundry-mobile.edpos.vn/api/orders/${orderId}/status`,
        method: "PUT",
      ...
      const paymentDetailsResponse = await fetch(`https://api-laundry-mobile.edpos.vn/api/orders/${orderId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
