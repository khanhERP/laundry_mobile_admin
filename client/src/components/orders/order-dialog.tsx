...
export function OrderDialog({
  ...
  const { data: existingOrderItems, refetch: refetchExistingItems } = useQuery({
    queryKey: ["https://laundry-mobie-admin.onrender.com/api/order-items", existingOrder?.id],
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest(
        "GET",
        `https://laundry-mobie-admin.onrender.com/api/order-items/${existingOrder.id}`,
      );
  ...
  const createOrderMutation = useMutation({
        ...
        if (mode === "edit" && existingOrder) {
          ...
          if (orderData.items.length > 0) {
            ...
            const addItemsResponse = await apiRequest(
              "POST",
              `https://laundry-mobie-admin.onrender.com/api/orders/${existingOrder.id}/items`,
              {
          ...
          if (shouldRecalculate) {
            ...
            try {
              const recalcResponse = await apiRequest(
                "POST",
                `https://laundry-mobie-admin.onrender.com/api/orders/${existingOrder.id}/recalculate`,
              );
            ...
            for (let i = 0; i < orderData.existingItems.length; i++) {
              ...
              try {
                const updateResponse = await apiRequest(
                  "PUT",
                  `https://laundry-mobie-admin.onrender.com/api/order-items/${item.id}`,
                  {
          ...
          const updateResponse = await apiRequest(
            "PUT",
            `https://laundry-mobie-admin.onrender.com/api/orders/${existingOrder.id}`,
            {
          ...
          const response = await apiRequest("POST", "https://laundry-mobie-admin.onrender.com/api/orders", orderData);
    ...
    onSuccess: async (response) => {
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ["https://laundry-mobie-admin.onrender.com/api/order-items"] }),
