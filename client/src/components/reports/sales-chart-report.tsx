...
export function SalesChartReport() {
  ...
  const {
    ...
    queryKey: [
      "https://api-laundry-mobile.edpos.vn/api/orders/date-range",
      startDate,
      endDate,
      startTime,
      endTime,
    ...
    queryFn: async () => {
      try {
        ...
        const response = await fetch(
          `https://api-laundry-mobile.edpos.vn/api/orders/date-range/${encodeURIComponent(startDateTimeISO)}/${encodeURIComponent(endDateTimeISO)}`,
        );
        if (!response.ok) {
  ...
  const { data: products } = useQuery({
    queryKey: ["https://api-laundry-mobile.edpos.vn/api/products", selectedCategory, productType, productSearch],
    queryFn: async () => {
      const response = await fetch(
        `https://api-laundry-mobile.edpos.vn/api/products/${selectedCategory}/${productType}/${productSearch || ""}`,
      );
  ...
  const { data: customers } = useQuery({
    queryKey: ["https://api-laundry-mobile.edpos.vn/api/customers", customerSearch, customerStatus],
    queryFn: async () => {
      const response = await fetch(
        `https://api-laundry-mobile.edpos.vn/api/customers/${customerSearch || "all"}/${customerStatus}`,
      );
  ...
  const { data: productAnalysisData, isLoading: productAnalysisLoading } =
    useQuery({
      queryKey: [
        "https://api-laundry-mobile.edpos.vn/api/product-analysis",
      ...
      queryFn: async () => {
        ...
        const response = await fetch(`https://api-laundry-mobile.edpos.vn/api/product-analysis?${params}`);
  ...
  const { data: transactions } = useQuery({
    queryKey: ["https://api-laundry-mobile.edpos.vn/api/transactions"],
    staleTime: 5 * 60 * 1000,
  });
