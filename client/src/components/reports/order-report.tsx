...
export function OrderReport() {
  ...
  const { data: orders = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch(`https://api-laundry-mobile.edpos.vn/api/orders/date-range/${startDate}/${endDate}`);
        if (!response.ok) {
  ...
  const { data: transactions = [] } = useQuery({
    queryKey: ["https://api-laundry-mobile.edpos.vn/api/transactions", startDate, endDate],
    queryFn: async () => {
      try {
        const response = await fetch(`https://api-laundry-mobile.edpos.vn/api/transactions/${startDate}/${endDate}`);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
