...
export function SupplierFormModal({ isOpen, onClose, supplier }: SupplierFormModalProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: InsertSupplier) => {
      const response = await apiRequest('POST', 'https://laundry-mobie-admin.onrender.com/api/suppliers', data);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: Partial<InsertSupplier>) => {
      const response = await apiRequest('PUT', `https://laundry-mobie-admin.onrender.com/api/suppliers/${supplier!.id}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['https://laundry-mobie-admin.onrender.com/api/suppliers'] });
