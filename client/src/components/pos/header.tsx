...
export function POSHeader({ onLogout }: POSHeaderProps) {
  ...
  const { data: todayAttendance } = useQuery<AttendanceRecord[]>({
    queryKey: ["https://api-laundry-mobile.edpos.vn/api/attendance", todayDate],
    queryFn: async () => {
      const response = await fetch(`https://api-laundry-mobile.edpos.vn/api/attendance?date=${todayDate}`);
      if (!response.ok) {
