...
export function EmployeeFormModal({
  ...
  const generateEmployeeId = async () => {
    try {
      const response = await apiRequest("GET", "https://api-laundry-mobile.edpos.vn/api/employees/next-id");
  ...
  const createMutation = useMutation({
    mutationFn: async (data: InsertEmployee) => {
      const response = await apiRequest("POST", "https://api-laundry-mobile.edpos.vn/api/employees", data);
      if (!response.ok) {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["https://api-laundry-mobile.edpos.vn/api/employees"] });
      toast({
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: InsertEmployee) => {
      const response = await apiRequest(
        "PUT",
        `https://api-laundry-mobile.edpos.vn/api/employees/${employee?.id}`,
        data,
      );
      if (!response.ok) {
        const errorData = await response.json();
