...
export default function PurchasesPage({ onLogout }: PurchasesPageProps) {
  ...
  const { data: purchaseOrders = [], isLoading: isOrdersLoading } = useQuery<PurchaseOrder[]>({
    queryKey: ["https://api-laundry-mobile.edpos.vn/api/purchase-receipts", { startDate, endDate, productFilter, searchTerm }],
    queryFn: async () => {
      ...
      const url = `https://api-laundry-mobile.edpos.vn/api/purchase-receipts${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (orderIds: number[]) => {
      return apiRequest("POST", "https://api-laundry-mobile.edpos.vn/api/purchase-receipts/bulk-delete", {
        orderIds
      });
    },
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["https://api-laundry-mobile.edpos.vn/api/purchase-receipts"] });
