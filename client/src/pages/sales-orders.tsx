...
export default function SalesOrdersPage({ onLogout }: SalesOrdersPageProps) {
    ...
    queryFn: async () => {
      try {
        console.log(`Dashboard - Date Range Query:`, {
          ...
          apiUrl: `https://laundry-mobie-admin.onrender.com/api/orders/date-range/${startDate}/${endDate}`,
        });

        const response = await fetch(
          `https://laundry-mobie-admin.onrender.com/api/orders/date-range/${startDate}/${endDate}`,
        );
        if (!response.ok) {
  ...
  const updateOrderStatus = useMutation({
    mutationFn: async ({
      ...
      const response = await fetch(`https://laundry-mobie-admin.onrender.com/api/orders/${orderId}/status`, {
        method: "PUT",
        headers: {
