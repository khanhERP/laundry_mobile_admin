...
export default function SuppliersPage({ onLogout }: SuppliersPageProps) {
  ...
  const { data: suppliers, isLoading } = useQuery({
    queryKey: ['https://api-laundry-mobile.edpos.vn/api/suppliers', { status: selectedStatus, search: searchQuery }],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (selectedStatus !== 'all') params.append('status', selectedStatus);
      if (searchQuery) params.append('search', searchQuery);
      
      const response = await apiRequest('GET', `https://api-laundry-mobile.edpos.vn/api/suppliers?${params}`);
  ...
  const { data: supplierStats } = useQuery({
    queryKey: ['https://api-laundry-mobile.edpos.vn/api/purchase-orders/supplier-stats'],
    queryFn: async () => {
      const response = await apiRequest('GET', 'https://api-laundry-mobile.edpos.vn/api/purchase-orders/supplier-stats');
      return response.json();
    },
  });
  ...
  const deleteSupplierMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest('DELETE', `https://api-laundry-mobile.edpos.vn/api/suppliers/${id}`);
      return response.json();
    },
    onSuccess: () => {
